...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function early_checks()
    {
        ...
        $data = array();
        $p_api_id     = $this->input->post('api_id');
        $p_api_key    = $this->input->post('api_key');
        $p_api_secret = $this->input->post('api_secret');

        $p_api_key = substr($p_api_key, 32);

        $ip = $_SERVER['REMOTE_ADDR'];
        
        if(!isset($p_api_id) || !isset($p_api_key) || !isset($p_api_secret))
        {
            $p_api_id = $p_api_id=="" ? "0" : $p_api_id;

            $log_message = "Auth Param not send completely";
            $log_status = "5";
            $this->insert_log_api_rest($p_api_id, "API Auth", $log_status, $log_message, $ip);

            $this->response(array('status'=>false, 'message'=>'APPS ID, APPS KEY, APPS SECRET Must Be Sent.'), 400);
        }

        $data = $this->Restmodel->_is_public_api($p_api_id, $p_api_key, $p_api_secret); 
        if(!isset($data['api_id']))
        {
            $p_api_id = $p_api_id=="" ? "0" : $p_api_id;
            
            $log_message = "Invalid Auth API";
            $log_status = "5";
            $this->insert_log_api_rest($p_api_id, "API Auth", $log_status, $log_message, $ip);

            $this->response(array('status'=>'401', 'message'=>'Invalid Authorize API , please make sure your access.'), 200);
        }
        
        $data['ip'] = $ip;

        $this->Restmodel->update_last_connect($p_api_id, $p_api_key, $p_api_secret);
    }

    function insert_log_api_rest($p_api_id, $p_action, $log_status, $log_message, $ip, $parameter_sent="")
    {
        $this->load->model('Restmodel');
        $this->Restmodel->insert_log_api_rest($p_api_id, $p_action, $log_status, $log_message, $ip, $parameter_sent);
    }
